...
namespace main_menu
{
	...
	struct KW_BUTTON_LOGIC_HEAD(home_button_logic)
	{
	...
	{
		self.get<transform>().set_position(vec3{ app::instance().window.win_width / 2, 400, 0 });

		auto& col = self.get<collider2d>();
		col.size = vec2{ 716, 160 };
		col.make_centered();


		self.emplace<sprite2d_bundle>()
			.add_centered(&renderer::textures["home_normal"], vec2{ 716.0f, 160.0f }, renderer::textures["home_normal"].get_texture_coords())
			.add_centered(&renderer::textures["home_highlighted"], vec2{ 716.0f, 160.0f }, renderer::textures["home_highlighted"].get_texture_coords())
			.add_centered(&renderer::textures["home_pressed"], vec2{ 716.0f, 160.0f }, renderer::textures["home_pressed"].get_texture_coords());

		self.emplace<button_component>().bind<home_button_logic>(&self);
